...
function checkForm(navegate2) {
  ...
  lapassword.textContent = "Crea Contrase\xF1a";
  ...
  btnSubmit.addEventListener("click", (e) => {
    e.preventDefault();
    loginWithCredentials(email.value, password.value).then(() => {
      navegate2("/welcome");
    }).catch((errorin) => {
      ...
      if (errorinCode === "auth/weak-password") {
        ...
        messengeErr.textContent = "Porvafor Ingrese una direcci\xF3n de correo electorinico";
      } else if (errorinCode === "auth/missing-password") {
        messengeErr.textContent = "Por favor Crea Contrase\xF1a";
...
function home() {
  ...
  paragraph.textContent = "Con\xE9ctate con otras Mam\xE1s y comparte experiencias \xFAnicas de la maternidad";
...
const savePost = (content, email) => uf(sh(db, "post"), { content, email, like: [] });
const getOnePost = (id2) => Jl(rh(db, "post", id2));
const activeLoad = (callback) => cf(sh(db, "post"), callback);
const deleteId = (id2) => of(rh(db, "post", id2));
const updataPost = (id2, newcontent) => rf(rh(db, "post", id2), newcontent);
const giveLike = (id2, email) => rf(rh(db, "post", id2), {
  like: Sf(email)
});
const disLike = (id2, email) => rf(rh(db, "post", id2), {
  like: Df(email)
});
function post() {
  const section = document.createElement("section");
...
<header>
  ...
  <div>
<button id="btnClouseLogin" type="button">cerrar sesi\xF3n</button>
</div>
</header>
...
<form id="post-publications">
  ...
  <input id="inputPublication" type="text" placeholder="\xBFC\xF3mo te sientes hoy?"></input>
  ...
  async function load() {
    activeLoad((querySnapshot) => {
      ...
      btnsLike.forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          getOnePost(classLike).then((res) => {
            ...
            if (dataOnePost.like !== void 0 && dataOnePost.like.includes(currentUser.email)) {
      ...
      btnDelete.forEach((btn) => {
        btn.addEventListener("click", (e) => {
          const messegeAlert = confirm("\xBFSeguro que quiere eliminar este post?");
          if (messegeAlert) {
...
function signInForm(navegate2) {
  ...
  const btnlogin = document.createElement("button");
  btnlogin.textContent = "Inicia Sesi\xF3n";
  ...
  labelPasswordL.textContent = "Ingrese su contrase\xF1a";
  ...
  btnlogin.addEventListener("click", (e) => {
    e.preventDefault();
    login(email2.value, password2.value).then(() => {
      navegate2("/welcome");
    }).catch((error2) => {
      ...
      if (errorCode === "auth/weak-password") {
        ...
        messengeErr2.textContent = "Porvafor Ingrese una direcci\xF3n de correo electorinico";
      } else if (errorCode === "auth/missing-password") {
        messengeErr2.textContent = "Por favor Ingrese Contrase\xF1a";
      } else if (errorCode === "auth/wrong-password") {
        messengeErr2.textContent = "Contrase\xF1a Invalida";
